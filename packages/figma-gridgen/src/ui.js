...
function toggleEditable(htmlTagId, isPrerequisiteSelected, defaultValue) {
  const htmlTagById = Figma.getHTMLInputElementById(htmlTagId);
...
/* Validate User Input */
function validateInput(mode, columns, rows, columnWidth, rowHeight) {
  let validInput = true;
  // reset invalid CSS
  resetInvalidInput();
  // negative and invalid value check
  const inputsForMode = defaultInputsForModes[mode];
  for (let input of inputsForMode) {
    const inputValue = Figma.getValue(input, "number");
...
const modes = Object.keys(defaultInputsForModes);
...
document.onkeydown = keyDown => {
  ...
  if (keyDown.key === "Shift") {
    ...
    let value = parseInt(activeElement.value, 10);
    ...
    // Selected Mode
    const mode = Figma.getValue("count-and-table-size", "boolean")
...
function processInputToMessage() {
  ...
  switch (mode) {
    case "count-and-table-size":
      columns = Figma.getValue("columns", "number");
      rows = Figma.getValue("rows", "number");
      columnWidth = Figma.getValue("tableWidth", "number") / columns;
      rowHeight = (Figma.getValue("tableHeight", "number") - headerHeight) / rows;
      break;
    case "count-and-cell-size":
      columns = Figma.getValue("columns", "number");
      rows = Figma.getValue("rows", "number");
      columnWidth = Figma.getValue("columnWidth", "number");
      rowHeight = Figma.getValue("rowHeight", "number");
      break;
    case "cell-and-table-size":
      tableWidth = Figma.getValue("tableWidth", "number");
      tableHeight = Figma.getValue("tableHeight", "number");
      columnWidth = Figma.getValue("columnWidth", "number");
      rowHeight = Figma.getValue("rowHeight", "number");
  ...
  // Validation
  const validWithinLimits = validateInput(mode, columns, rows, columnWidth, rowHeight);
